...
def start_docker(uid, token):
    ...
    if stdlog:
        ...
        subprocess.Popen(
            f"docker logs -f {child_docker_name}", shell=True, stdout=f, stderr=f)#todo: use better way to redirect logs
        time.sleep(0.1)
    if external_proxy_port:
        # os.system(
        #     f"docker cp {data_dir}/vol/socat {child_docker_name}:/socat")
        # time.sleep(0.1)
        os.system(
                ...
                if uid != None:
                    ...
                    if dockerinfo != None:
                        ...
                        try:
                            ...
                            transport = httpx.HTTPTransport(uds='/vol/sock/'+ghost+'/socat.sock')
                            client = httpx.Client(transport=transport)
                            r = client.get("http://"+HOST_PREFIX+ghost+DOMAIN+PROB_PATH, timeout=1, follow_redirects=False)
                            code = r.status_code
...
if __name__ == '__main__':
    assert challenge_docker_name.endswith("-challenge") or challenge_docker_name.endswith("_challenge")
    assert data_dir != ""
